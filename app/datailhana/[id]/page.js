...
export default async function detailehana(props){
    ...
    //     return arr.findIndex(item => item.productName === obj.productName) === index;
    //   });
      const resultd = optional.find(item => item.fin_prdt_nm === productName);

    console.log(resultd.kor_co_nm);
    ...
    return(
        ...
        <div className="explanbody">
            <div className="expalnebox">
                <p id="explaneTitle">{resultd.fin_prdt_nm}</p>
                <p id="explaneContent">{resultd.mtrt_int}</p>
            </div>
        </div>
        <div className="deposit3dbox">
            <div className="deposit3dBox1"> 
                ...
                <Image src={`/${resultd.kor_co_nm}.png`} alt={`${resultd.kor_co_nm}.png`} priority={true} width="180" height="180" className='deposit3dimg'/>
                </div>
                <div className="deposit3dBox2">
                    <div className="d3d">
                        <button className="deposit3dButton">{resultd.intr_rate_type_nm}</button>
                        <button className="deposit3dButton1">{resultd.rsrv_type_nm}</button>
                        <button className="deposit3dButton1">{resultd.fin_prdt_nm}</button>
                    </div>
                        <div className="d3dB">
                            <div className="d3dBT">{resultd.fin_prdt_nm}</div>
                                <div className="d3dBTdT">{resultd.intr_rate2}</div>
                            ...
                            <div className="d3dBTI">
                                    ...
                                    <div className="d3dBTID">({resultd.save_trm}개월)</div>
                            </div>
                        </div>
                        <div className="d3dbD">
                            <div>{resultd.mtrt_int}</div>
                        </div>
                </div>
            </div>
            <div>
             <ul className="deposit3dID">
                ...
                <li className="deposit3dIDC">{resultd.join_way}</li>
                <li className="deposit3dIDC">{resultd.mtrt_int}</li>
                <li className="deposit3dIDC">{resultd.spcl_cnd}</li>
            </ul>
            </div>
            <p className="deposit3dII">{resultd.etc_note}</p>
        </div>
